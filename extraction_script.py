...
def extract_data_from_file(filename):
    df = pd.DataFrame(columns=['n_scientists', 'temp', 'bias','dims', 'sparsity', 'datapoints', 'exp_cap', 'bandwidth', 'max_exchange',
                                'sociality','ensemble_div', 'com_div_abs_avg', 'com_div_abs_med', 'gini', 'accuracy', 'last_acc',
                                'naive_subj', 'naive_obj', 'avg_MAPE', 'best_MAPE', 'worst_MAPE', 'rc_ensemble_div', 'rc_com_div',
                                'PIC', 'AMI', 'avg_rc_n_coms', 'max_rc_n_coms', 'min_rc_n_coms', 'final_n_coms', 'n_coms_time',
                            'med_obj', 'best_obj', 'worst_obj', 'med_subj', 'best_subj', 'worst_subj',
                            'CrPC_last', 'avg_rc_CrPC', 'med_MAPE', 'q25_MAPE', 'q75_MAPE'])
    ...
    ensemble_div = str([div[0] for div in diversity_time])
    com_div = [div[1] for div in diversity_time]

    def remove_zero_diversities(community_diversities):
        return {community: diversity for community, diversity in community_diversities.items() if diversity != 0}
    com_div_abs = [remove_zero_diversities(c) for c in com_div]
    com_div_abs_last = com_div_abs[-1]
    com_div_abs_avg = np.mean(list(com_div_abs_last.values()))
    com_div_abs_med = np.median(list(com_div_abs_last.values()))


    com_dict = str([com_time])
    accuracy = str(accuracy_time)
    last_acc = accuracy_time[-1]
    ensemble_n_subj = str([np.mean(scores) for scores in naive_subjective])
    ensemble_n_obj = str([np.mean(scores) for scores in naive_objective])

    ensemble_divs = [div[0] for div in diversity_time]
    com_divs = [np.mean(list(div[1].values())) for div in diversity_time]
    naive_subj = [np.mean(scores) for scores in naive_subjective]
    time5 = [i*5 for i in range(len(ensemble_divs))]
    time25 = [i*25 for i in range(len(naive_subj))]
    rc_ensemble_div = average_rate_of_change(ensemble_divs, time5)[0]
    rc_com_div = average_rate_of_change(com_divs, time5)[0]
    rc_naive_subj = average_rate_of_change(naive_subj, time25)[0]
    PIC = Evaluation.PIC(naive_objective[-1], com)

    a_graphs = []
    for g in graph_time:
      a_graphs.append(graph_to_json_string(g))


    last_graph = graph_time[-1]
    gini = Evaluation.gini_coefficient(last_graph)

    med_subj, med_obj, best_subj, best_obj, worst_subj, worst_obj = Evaluation.naive_eval(scientists, env, com)


    # *** Specialisation eval:
    mape, best_MAPE, worst_MAPE, all_MAPE = Evaluation.voronoi_specialisation_communities(scientists, com, env)

    med_MAPE = np.median(all_MAPE)
    q2_MAPE = np.percentile(all_MAPE, 2.5)
    q9_MAPE = np.percentile(all_MAPE, 97.5)

    # *** AMI:
    ami = Evaluation.mutual_information(scientists, env, com)

    com_times = state['community_dicts']

    com = state['community']
    coms_stored = state['coms_stored']
    naive_objective_time = state['naive_objective']
    naive_subjective_time = state['naive_subjective']

    n_coms_time = [len(set(com_time.values())) for com_time in com_times]
    time5 = [i*5 for i in range(len(com_times))]
    time25 = [i*25 for i in range(len(naive_objective_time))]
    avg_rc_n_coms, rc_n_coms = average_rate_of_change(n_coms_time, time5)
    max_rc_n_coms = np.max(rc_n_coms)
    min_rc_n_coms = np.min(rc_n_coms)
    final_n_coms = n_coms_time[-1]

    CrPC_time = []
    for i in range(len(coms_stored)):
      CrPC_time.append(Evaluation.CrPC(naive_objective_time[i], coms_stored[i]))
    CrPC_last = CrPC_time[-1]
    avg_rc_CrPC = average_rate_of_change(CrPC_time, time25)[0]


    # enter things into the database:
    df.loc[trial_num] = [len(scientists), settings['temp'], settings['bias'], settings['dims'], settings['sparsity'],
                        settings['n_datapoints'], settings['explanation_capacity'], settings['scope_bandwidth'],
                        settings['max_exchange'], settings['percentage_swaps'], ensemble_div, com_div_abs_avg, com_div_abs_med,
                        gini, accuracy, last_acc, ensemble_n_subj, ensemble_n_obj, mape, best_MAPE, worst_MAPE, rc_ensemble_div,
                        rc_com_div, PIC, ami, avg_rc_n_coms, max_rc_n_coms, min_rc_n_coms, final_n_coms, str(n_coms_time),
                        med_obj, best_obj, worst_obj, med_subj, best_subj, worst_subj,
                      CrPC_last, avg_rc_CrPC, med_MAPE, q2_MAPE, q9_MAPE]
...
def main():
    ...
    output_file = "{}_processed.csv".format(base_name)
    output_dir = os.path.join(os.environ['HOME'], 'CJ', 'pickle', 'processed_files')
